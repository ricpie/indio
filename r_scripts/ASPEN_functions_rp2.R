...
read_aspen_header <- function(file, long_format=F){
  ...
  df <- df %>%
        dplyr::mutate(across(c("COserial","NO2serial","O3serial","CO2Slope","CO2Intercept",
        ...
        dplyr::mutate(across(c("StartDateTimeUTC","Filter1StartDateTimeUTC","Filter2StartDateTimeUTC",
