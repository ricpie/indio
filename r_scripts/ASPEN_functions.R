...
read_aspen_header <- function(file, long_format=F){
  ...
  header_lines <- readLines(file, n=70, skipNul=T)
  
  n_max    <- grep("Filter2AverageVolumetricFlowRate", header_lines)
  
  df <- read.csv(file, nrows=n_max, stringsAsFactors=F, blank.lines.skip=F) %>%
        dplyr::filter(!(PARAMETER %in% c("","SAMPLE IDENTIFICATION","Calibration Coefficients","SETUP SUMMARY","SAMPLE SUMMARY","SAMPLE LOG"))) %>%
  ...
  df <- df %>%
        dplyr::mutate(across(any_of(c("COserial","NO2serial","O3serial","CO2Slope","CO2Intercept",
                                      ...
                                      "ASPENSampledRuntime","HHBSampledRuntime","Filter1SampledRuntime","Filter2SampledRuntime",
                                      "Filter1AverageVolumetricFlowRate","Filter2AverageVolumetricFlowRate")), as.numeric)) %>%
        dplyr::mutate(across(c("StartDateTimeUTC","EndDateTimeUTC","Filter1StartDateTimeUTC","Filter2StartDateTimeUTC",
...
read_aspen <- function(file){
  ...
  df_h <- read_aspen_header(file) %>% 
          dplyr::select(any_of(c("LogFilename","ASPENserial","HHBserial","SampleName","HGserial","COserial","NO2serial","O3serial","CO2serial")))
  
  # Read some lines, find the line with the column headers, the subtract 1 to get the number of lines to skip
  header_lines <- readLines(file, n=76)
  nskip        <- grep("SampleTime", header_lines) - 1
