...
DT::datatable(field_log %>% 
                dplyr::filter(!grepl("blank",excel_Comments,ignore.case = T),
                              is.na(`excel_Quartz Blank ID`)) %>% 
...
aspen_header_merged1 <- dplyr::left_join(aspen_header_merged_temp,filter_list, by="CartridgeID") %>% 
  ...
  dplyr::mutate(excel_sent_datediff = as.Date(DateSent)-as.Date(excel_datetime_start), 
...
aspen_header_merged <- aspen_header_merged1 %>% 
  ...
  dplyr::group_by(CartridgeID,file_start_date) %>% 
