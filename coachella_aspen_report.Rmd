...
aspen_header_merged_temp = dplyr::full_join(aspen_header,field_log_filter_list,by = c("cartridge_id")) %>% 
  ...
  dplyr::mutate(type_match_log_headers = grepl(substr(excel_type,1,3),tolower(substr(SizeCut,1,3)))) %>% 
  ...
  dplyr::arrange(cartridge_id,desc(sampletype),file_start_date,excel_file_datediff) %>% 
...
write.xlsx(list(complete_list = aspen_header_merged_temp,clean_list = aspen_header_merged),file = filename)
